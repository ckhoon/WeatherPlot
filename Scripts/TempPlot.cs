...
public class TempPlot{
	...
	public void Init(string strJson)
	{
		if ( storeStations(strJson) )
			...
			Debug.LogError("Error getting first data for temp");
	}
	...
	private bool storeStations(string strJson)
	{
		...
		Debug.Log("total temp stations - " + listStations.Count);
		return true;
	}
	...
	private bool loadData(string strJson)
	{
		...
		for ( int i = 0; i < data["items"][0]["readings"].Count; i++ )
		{
			...
			entry[Headers.Temp] = data["items"][0]["readings"][i]["value"].AsFloat;
